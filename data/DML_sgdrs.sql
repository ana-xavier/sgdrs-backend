...
INSERT INTO item(id_item, nome, descricao, quantidade, categoria, id_centro) VALUES
    (gen_random_uuid(), 'Arroz', 'Pacote de arroz 5kg', 100, 'Alimento', (SELECT id_centro FROM centro_distribuicao WHERE nome = 'Centro São Paulo')),
    (gen_random_uuid(), 'Feijão', 'Pacote de feijão 1kg', 200, 'Alimento', (SELECT id_centro FROM centro_distribuicao WHERE nome = 'Centro Rio de Janeiro')),
    (gen_random_uuid(), 'Cobertor', 'Cobertor de lã', 50, 'Vestuário', (SELECT id_centro FROM centro_distribuicao WHERE nome = 'Centro Belo Horizonte')),
    (gen_random_uuid(), 'Camiseta', 'Camiseta de algodão', 150, 'Vestuário', (SELECT id_centro FROM centro_distribuicao WHERE nome = 'Centro Salvador')),
    (gen_random_uuid(), 'Sapato', 'Sapato de couro', 75, 'Calçados', (SELECT id_centro FROM centro_distribuicao WHERE nome = 'Centro Recife'));
